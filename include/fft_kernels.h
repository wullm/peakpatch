...
static inline void kernel_dx_inv_poisson(struct kernel *the_kernel) {
    double kx = the_kernel->kx;
    double k = the_kernel->k;
    the_kernel->kern = (k > 0) ? -I * kx / (k * k) : 0.0;
}

static inline void kernel_dy_inv_poisson(struct kernel *the_kernel) {
    double ky = the_kernel->ky;
    double k = the_kernel->k;
    the_kernel->kern = (k > 0) ? -I * ky / (k * k) : 0.0;
}

static inline void kernel_dz_inv_poisson(struct kernel *the_kernel) {
    double kz = the_kernel->kz;
    double k = the_kernel->k;
    the_kernel->kern = (k > 0) ? -I * kz / (k * k) : 0.0;
}

static inline void kernel_dx_dx_inv_poisson(struct kernel *the_kernel) {
    double kx = the_kernel->kx;
    double k = the_kernel->k;
    the_kernel->kern = (k > 0) ? kx * kx / (k * k) : 0.0;
}

static inline void kernel_dx_dy_inv_poisson(struct kernel *the_kernel) {
    double kx = the_kernel->kx;
    double ky = the_kernel->ky;
    double k = the_kernel->k;
    the_kernel->kern = (k > 0) ? kx * ky / (k * k) : 0.0;
}

static inline void kernel_dx_dz_inv_poisson(struct kernel *the_kernel) {
    double kx = the_kernel->kx;
    double kz = the_kernel->kz;
    double k = the_kernel->k;
    the_kernel->kern = (k > 0) ? kx * kz / (k * k) : 0.0;
}

static inline void kernel_dy_dy_inv_poisson(struct kernel *the_kernel) {
    double ky = the_kernel->ky;
    double k = the_kernel->k;
    the_kernel->kern = (k > 0) ? ky * ky / (k * k) : 0.0;
}

static inline void kernel_dy_dz_inv_poisson(struct kernel *the_kernel) {
    double ky = the_kernel->ky;
    double kz = the_kernel->kz;
    double k = the_kernel->k;
    the_kernel->kern = (k > 0) ? ky * kz / (k * k) : 0.0;
}

static inline void kernel_dz_dz_inv_poisson(struct kernel *the_kernel) {
    double kz = the_kernel->kz;
    double k = the_kernel->k;
    the_kernel->kern = (k > 0) ? kz * kz / (k * k) : 0.0;
}

static inline void kernel_power_no_transfer(struct kernel *the_kernel) {
    // const struct cosmology *cosmo =
    //     (const struct cosmology *)the_kernel->params;
    double k = the_kernel->k;
    // double kern = sqrt(primordialPower(k, cosmo));
    double kern = sqrt(pow(k, -1.5));
...
//     } else {
//         /* Unpack the spline parameters */
//         struct spline_params *sp = (struct spline_params *)
//         the_kernel->params; const struct perturb_spline *spline = sp->spline;
...
//
//         /* Evaluate the transfer function for this tau and k */
//         the_kernel->kern = perturbSplineInterp(spline, k_index, tau_index,
//         u_k, u_tau, index_src);
//     }
// }
//
...
//
//         /* Evaluate the transfer function for this tau and k */
//         the_kernel->kern = 1.0 / perturbSplineInterp(spline, k_index,
//         tau_index, u_k, u_tau, index_src);
//     }
// }
...
static inline void kernel_undo_Hermite_window(struct kernel *the_kernel) {
    ...
    if (the_kernel->k == 0) {
        ...
        struct Hermite_kern_params *p =
            (struct Hermite_kern_params *)the_kernel->params;
        int order = p->order;
